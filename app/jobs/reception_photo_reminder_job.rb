...
class ReceptionPhotoReminderJob < ApplicationJob
  ...
  def perform(service_job_id)
    ...
    # Каналы напоминают по-разному, поэтому dedup_scope прикрывает только
    # колокольчик: запись там висит непрочитанной, пока её не закроют, и второй
    # такой же строки не нужно — достаточно переподнять существующую. Сообщение
    # в Telegram, наоборот, уходит вверх ленты и напомнить о себе может только
    # новым сообщением, поэтому перезапуск джобы отправляет его снова. Картинка
    # едет подписью, так что push остаётся один.
