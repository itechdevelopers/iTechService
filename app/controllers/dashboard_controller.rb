class DashboardController < ApplicationController
  ...
  def ready_service_jobs
    @service_jobs = ServiceJob.ready(current_department).order(done_at: :desc)
                      .search(action_params).page(params[:page])
    respond_to do |format|
      format.js
    end
  end
  ...
  def load_actual_jobs
    ...
    if current_user.able_to?(:print_receipt)
      @service_jobs = @service_jobs.search(action_params).newest.page(params[:page])
    else
      @service_jobs = @service_jobs.search(action_params).oldest.page(params[:page])
    end
  end

  def load_actual_orders
    if current_user.technician?
      @orders = policy_scope(Order).actual_orders.technician_orders.search(action_params).oldest.page(params[:page])
    else
      @orders = policy_scope(Order).actual_orders.marketing_orders.search(action_params).oldest.page(params[:page])
    end
  end
end
