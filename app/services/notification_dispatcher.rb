...
class NotificationDispatcher
  ...
  # Повод, о котором уже уведомляли. В колокольчике запись висит, пока её не
  # закроют, поэтому вторую не создаём — переброадкастим существующую, и иконка
  # снова привлекает внимание. Telegram так «переподнять» нельзя: там напомнить
  # о себе можно только новым сообщением, и оно уходит независимо от дедупа.
