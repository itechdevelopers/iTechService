...
# Validates whether a user can be assigned to close the store for a given department+date.
# Same checks as DutyAssignmentValidator but WITHOUT duty/cashier blocking.
#
# Returns: { eligible: true/false, warnings: [...] }
class StoreClosingAssignmentValidator
  ...
  def validate
    ...
    if user.is_fired?
      warnings << I18n.t('store_closing_schedules.warnings.employee_fired')
      return { eligible: false, warnings: warnings }
    end

    entry = find_schedule_entry
    unless entry
      warnings << I18n.t('store_closing_schedules.warnings.no_schedule_entry')
      return { eligible: false, warnings: warnings }
    end

    unless entry.occupation_type&.counts_as_working?
      warnings << I18n.t('store_closing_schedules.warnings.not_working')
      return { eligible: false, warnings: warnings }
    end

    if entry.department_id != department.id
      warnings << I18n.t('store_closing_schedules.warnings.different_department',
                          department: entry.department&.name)
      return { eligible: false, warnings: warnings }
    end

    # No duty/cashier blocking — they are only shown as markers
  ...
  def validate_evening_shift(entry, warnings)
    ...
    if dept_hours.nil? || dept_hours.is_closed?
      warnings << I18n.t('store_closing_schedules.warnings.department_closed')
      return { eligible: false, warnings: warnings }
    end
    ...
    unless shift_end
      warnings << I18n.t('store_closing_schedules.warnings.no_evening_shift')
      return { eligible: false, warnings: warnings }
    end
    ...
    if shift_end.seconds_since_midnight >= dept_hours.closes_at.seconds_since_midnight
      ...
      warnings << I18n.t('store_closing_schedules.warnings.shift_ends_before_closing',
