...
RSpec.describe KpiAuditPolicy do
  it 'allows only existing superadmins' do
    superadmin = double(role: 'superadmin', superadmin?: true)
    manager = double(role: 'manager', superadmin?: false)
    employee = double(role: 'software', superadmin?: false)

    expect(described_class.new(superadmin, :kpi_audit).index?).to eq(true)
    expect(described_class.new(manager, :kpi_audit).index?).to eq(false)
