...
def deliver(path, connector):
    ...
    body=json.dumps({'delivery_id':identity,'report':report},ensure_ascii=False).encode()
    ...
    with opener.open(req,timeout=60) as response:
        ...
        if response.status not in (200,201) or result.get('ok') is not True:raise ValueError('delivery_rejected')
...
if __name__=='__main__':
    ...
    try:print(json.dumps(deliver(args.report,args.connector)))
